...
{
    public class ReportAgentSubmissionController : ApiController
    {
        [HttpGet]
        public IHttpActionResult GetAll(int currentPage, int pageSize, string sortColumn, bool sortInAsc, string searchParams)
        {
            try
            {
                ...
                using (var db = new BroadbandZoneEntities())
                {
                    ...
                    var results = (new BroadbandZoneEntities()).ReportAgentSubmissionStatus(currentPage,
                                                                                            pageSize,
                                                                                            sortColumn,
                                                                                            sortInAsc,
                                                                                            filterBy.StartDate,
        ...
        [HttpPost]
        [Route("api/ReportAgentSubmission/Download")]
        public HttpResponseMessage Download([FromBody] DateRange filterBy)
        {
            try
            {
                using (var db = new BroadbandZoneEntities(true))
                {
                    ...
                    {
                        //DateRange filterBy = JsonConvert.DeserializeObject<DateRange>(searchParams);
                        var results = db.ReportAgentSubmissionStatusDownload(filterBy.StartDate, filterBy.EndDate).ToList();
                        return ExcelHelper.ReadDataToExcel<ReportAgentSubmissionStatusDownload_Result>(results, "AgentSubmission");
                    }
                }
            }
            catch (Exception ex)
