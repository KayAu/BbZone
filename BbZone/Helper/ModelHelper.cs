...
namespace BroadbandZone_App.Helper
{
    public static class ModelHelper
    {
        ...
        public static Gridview<T> GetListdata<T>(Func<int?, int?, string, bool?, ObjectParameter, ObjectResult<T>> callDbMethod, int currentPage = 1, int pageSize = Constants.DefaultPageSize, string sortColumn = "", bool sortInAsc = false)
        {
            try
            {
                using (var db = new BroadbandZoneEntities())
                {
                    ObjectParameter totalRecord = new ObjectParameter("oTotalRecord", typeof(int));
                    var results = callDbMethod(currentPage, pageSize, sortColumn, sortInAsc, totalRecord).ToList();
                    return new Gridview<T>()
                    {
                        DisplayData = results,
                        TotalRecords = Convert.ToInt32(totalRecord.Value)
